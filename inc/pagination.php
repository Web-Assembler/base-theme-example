 ...
 */
function webassembler_pagination( $args = array(), $class = 'pagination' ) {
	...
	$args = wp_parse_args(
		...
		array(
			...
			'end_size'           => 1,
			'prev_next'          => true,
			'prev_text'          => __( '&laquo;', 'webassembler-base' ),
			'next_text'          => __( '&raquo;', 'webassembler-base' ),
			'type'               => 'array',
			'current'            => max( 1, get_query_var( 'paged' ) ),
			'screen_reader_text' => __( 'Posts navigation', 'webassembler-base' ),
		)
	);
	...
	<nav aria-labelledby="posts-nav-label" class="pagination__wrapper">
		...
		<ul class="<?php echo esc_attr( $class ); ?>">
			...
			foreach ( $links as $key => $link ) {
				...
				<li class="page-item <?php echo strpos( $link, 'current' ) ? 'active' : ''; ?>">
					<?php echo str_replace( 'page-number', 'page-link', $link ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</li>
				<?php
			}
			?>
